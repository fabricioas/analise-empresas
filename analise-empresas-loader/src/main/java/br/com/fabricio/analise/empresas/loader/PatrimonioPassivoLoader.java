...
public class PatrimonioPassivoLoader extends FileLoader<DemonstrativoFinanceiro> {

	public PatrimonioPassivoLoader(String repositorio, DataBaseService baseService) {
	...
	private List<ArquivoProcessamento> getArquivos(List<ArquivoProcessamento> list, Integer ano) {
		list.add(new ArquivoProcessamento(getRepositorio() + EnumRelatorios.BPP_ITR_CONSOLIDADO.getPath(ano),EnumRelatorios.BPP_ITR_CONSOLIDADO, EnumTipoDemonstrativo.CONSOLIDADO));
		list.add(new ArquivoProcessamento(getRepositorio() + EnumRelatorios.BPP_ITR_INDIVIDUAL.getPath(ano),EnumRelatorios.BPP_ITR_INDIVIDUAL, EnumTipoDemonstrativo.INDIVIDUAL));
		list.add(new ArquivoProcessamento(getRepositorio() + EnumRelatorios.BPP_INDIVIDUAL.getPath(ano),EnumRelatorios.BPP_INDIVIDUAL, EnumTipoDemonstrativo.INDIVIDUAL));
		list.add(new ArquivoProcessamento(getRepositorio() + EnumRelatorios.BPP_CONSOLIDADO.getPath(ano),EnumRelatorios.BPP_CONSOLIDADO, EnumTipoDemonstrativo.CONSOLIDADO));
		return list;
	}
